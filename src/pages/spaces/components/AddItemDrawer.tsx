...
export interface AddItemDrawerRef {
  open: (storageId: string) => void
}

interface AddItemDrawerProps {
  onAddItem: (pointId: string, item: StorageItem) => void
}

export const AddItemDrawer = forwardRef<AddItemDrawerRef, AddItemDrawerProps>(
  ({ onAddItem }, ref) => {
    const [isOpen, setIsOpen] = useState(false)
    const [activeStorageId, setActiveStorageId] = useState<string | null>(null)

    useImperativeHandle(ref, () => ({
      open: (storageId: string) => {
    ...
    const { mutate, isPending } = useMutation({
      mutationFn: async (data: ItemFormData) => {
        if (!activeStorageId) throw new Error('No active storage')
        const item: StorageItem = {
          id: Date.now().toString(),
          storagePointId: activeStorageId,
          name: data.name,
          type: data.type,
      ...
      onSuccess: data => {
        if (activeStorageId) {
          onAddItem(activeStorageId, data)
        }
    ...
    return (
      <Sheet open={isOpen} onOpenChange={handleClose}>
        <SheetContent side='bottom' className='h-[80vh] md:h-auto md:side-right md:w-125'>
          <SheetHeader>
...
AddItemDrawer.displayName = 'AddItemDrawer'
